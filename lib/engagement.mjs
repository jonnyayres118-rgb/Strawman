...
export function engagementFromOpportunity(opportunity,recommendation){return createEngagement({account:opportunity.company,outcome:recommendation.outcome||opportunity.whyElastic||opportunity.hypothesis||"",context:[opportunity.trigger,opportunity.tension,opportunity.hypothesis].filter(Boolean).join("\n\n"),members:(recommendation.members||[]).map(m=>({...m,id:id("member")})),deployment:recommendation.deployment||{weeks:8}});}
