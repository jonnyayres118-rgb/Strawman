...
export function calculateCommercialScenario(scenario = {}) {
  ...
  const missingInputs = members.filter(member => member.rateStatus !== 'VERIFIED').map(member => member.roleId);
  ...
  const isFullyCosted = missingInputs.length === 0;
  return {
    ...scenario,
    members,
    missingInputs,
    ...
    gm:isFullyCosted && verifiedRevenue ? verifiedGp / verifiedRevenue : null
  };
}
