...
Every BUY and SELL value carries provenance independently. Input precedence is explicit: a deal override wins for the side it overrides; otherwise actual talent BUY can replace benchmark BUY; otherwise the recovered/approved benchmark is used. A SELL override can be applied independently without destroying BUY provenance. If either side remains absent, the model stays MISSING_INPUT and does not infer the missing value.
...
A scenario can contain unresolved roles while being designed. The internal workspace shows verified subtotal economics, the unique missing role types and an unresolved-role count for the UI. A final scenario investment is unavailable until every included role is fully costed. Once complete, the internal view exposes final revenue, cost, GP and GM. Client proposal generation receives only the final investment and client-safe team/deployment fields such as role, layer, phase, total days, weeks and days per week. Individual BUY/SELL rates never cross that boundary.
